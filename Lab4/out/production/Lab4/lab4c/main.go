...
func routeFinder(graph *Graph, cities []string) {
	from := cities[rand.Int()%len(cities)]
	to := cities[rand.Int()%len(cities)]
	price := DFS(from, graph, cities, to)
...
func DFS(startingNode string, graph *Graph, cities []string, goalNode string) int {
...
func main() {
	cities := []string{"Kyiv", "Odesa", "Lviv", "Mariupol", "Vinnytsia", "Zhytomyr", "Dnipro", "Kharkiv", "Kherson", "Simferopol", "Ivankiv"}
	graph := Graph{}
	graph.init()
	...
	graph.addRoute("Lviv", "Mariupol", 3)
	graph.addRoute("Kyiv", "Vinnytsia", 10)
	graph.addRoute("Vinnytsia", "Zhytomyr", 3)
	graph.addRoute("Vinnytsia", "Kharkiv", 10)
	graph.addRoute("Kyiv", "Kherson", 1)
	graph.addRoute("Kherson", "Simferopol", 1)
	graph.addRoute("Mariupol", "Dnipro", 1)

	//go performPriceEditor(&graph, cities)
	//go performRouteEditor(&graph, cities)
	//go performCityEditor(&graph, cities)
	performRouteFinder(&graph, cities)
